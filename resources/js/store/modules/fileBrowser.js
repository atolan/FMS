  ...
  getTrash: ({ commit, getters }) => {
    ...
    axios
      ...
      .then(response => {
        ...
        commit('SET_TRASH_CNT', response.data.length)
...
const mutations = {
  ...
  SORT_DATA(state) {
    ...
    const field = JSON.parse(localStorage.getItem('sorting')).field
    const sort = JSON.parse(localStorage.getItem('sorting')).sort
