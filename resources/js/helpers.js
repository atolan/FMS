...
const Helpers = {
  install(Vue) {
      ...
      return options.map(role => {
        ...
        return {
          label: isArray(role.label) ?
            i18n.t(key, values) : i18n.t(role.label),
          value: role.value,
    ...
    Vue.prototype.$copyFunc = async function(to_id) {
      this.$store.commit('ADD_ITEMS_TO_COPYQUEUE', store.getters.clipBoard)
      await store.getters.clipBoard.forEach(data => {
        store.dispatch('copyItem_1', { data: data, to_id: to_id })
      })
      this.$store.commit('CLEAR_FILEINFO_DETAIL')
      this.$store.commit('CLEAR_CLIPBOARD')
    }

    Vue.prototype.$loadNextFolderContent = async function(folder) {
      await store.dispatch('getNextFolder', [{ folder: folder }])
    }
