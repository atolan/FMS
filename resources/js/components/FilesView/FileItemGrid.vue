<template>
    <div class="file-wrapper" @click="clickedItem" @dblclick="goToItem" spellcheck="false">
        <!--Grid preview-->
        <div :draggable="canDrag" @mousedown="onMouseDown" @dragstart="$emit('dragstart')" @drop="
				drop()
            ...
            <div class="icon-source">
                ...
                <div :class="{'check-select-folder' : this.source.type === 'folder', 'check-select' : this.source.type !== 'folder'}" v-if="multiSelectMode">
...
export default {
    ...
    computed: {
        ...mapGetters([
            'FilePreviewType', 'sharedDetail', 'fileInfoDetail', 'copyOrCutInfo', 'clipBoard', 'fileInfoPreview', 'data', 'focusNameID', 'treefolders'
        ]),
        itemid() {
            ...
            if (!this.$isMobile()) {
                ...
                if (this.source.type === 'folder') {
                    ...
                    // parents.forEach(element => {
                        events.$emit('leftMenu:show', this.source)
                    // });
